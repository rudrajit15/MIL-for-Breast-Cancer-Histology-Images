# -*- coding: utf-8 -*-
"""
Created on Sat Oct 27 12:46:15 2018
...
def UNet(x_train,x_val):
    ...
    #model.compile(optimizer = Adam(decay = 0.8), loss = 'sparse_categorical_crossentropy', metrics = ['accuracy'])
    nadam = keras.optimizers.Nadam(lr=0.008, beta_1=0.9, beta_2=0.999, epsilon=None, schedule_decay=0.004)
    #model.compile(optimizer = nadam, loss = 'sparse_categorical_crossentropy', metrics = ['accuracy'])
    model.compile(loss = SSIM_objective, optimizer = 'adam')
    ...
    model.fit_generator(datagen_train.flow(x_train, x_train, batch_size=batch_size),
                    steps_per_epoch=len(x_train) / batch_size,
                    epochs=epochs,
                    shuffle=True,
                    callbacks=[checkpointer,csv_log],
                    validation_data = datagen_val.flow(x_val, x_val, batch_size=batch_size),
